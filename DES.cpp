...
unsigned char* DES::encrypt(const unsigned char* plaintext)
{
	...
	//1. Declate an array DES_LONG block[2];
	DES_LONG block[2];

	//2. Use ctol() to convert the first 4 chars into long; store the result in block[0]
	block[0] = ctol(plaintxt);

	//3. Use ctol() to convert the second 4 chars into long; store the resul in block[1]
	block[1] = ctol(plaintxt + 4);
	...
	//7. Save the results in the the dynamically allocated char array
...
{
	//LOGIC:
	//Check to make sure that the block is exactly 8 characters (i.e. 64 bits)

	unsigned char ciphertxt[8];
	unsigned char decrypted_text[8];

	for(int i = 0; i < 8; i++)
		ciphertxt[i] = ciphertext[i];

	//1. Declate an array DES_LONG block[2];
	DES_LONG block[2];

	//2. Use ctol() to convert the first 4 chars into long; store the result in block[0]
	block[0] = ctol(ciphertxt);

	//3. Use ctol() to convert the second 4 chars into long; store the resul in block[1]
	block[1] = ctol(ciphertxt + 4);
